...
export default async (req: NextApiRequestWithSession, res: NextApiResponse): Promise<void> => {
    ...
    // 1. filter out multi-operator external products since they have a separate export page
    const productsWithoutMultiOperatorExternal = products.filter((product) => product.fareType !== 'multiOperatorExt');
    const activeOrPendingProducts = getActiveOrPendingProducts(productsWithoutMultiOperatorExternal);
