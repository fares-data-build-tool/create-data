...
const Home = ({ titlePrefix }: HomeProps): ReactElement => {
    const title = `${titlePrefix} Create Fares Data`;
    return (
...
export const getServerSideProps = async (ctx: NextPageContextWithSession): Promise<{}> => {
    ...
    const modalAttribute = getSessionAttribute(ctx.req, MULTI_MODAL_ATTRIBUTE);
    const titlePrefix = modalAttribute && modalAttribute.modes.length > 0 ? modalAttribute.modes.join(', ') : '';
