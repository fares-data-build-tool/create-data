            ...
            <div id="main">
              <div>
                ...
                <div>
                  ...
                  >
                    One or more multi-operator products have been completed in the last 24 hours.
                    Visit the <a href="https://${serviceDomain}/products/multiOperatorProductsExternal">multi-operator dashboard</a> to review your completed products.
                  </p>
                </div>
              </div>
            </div>
...
export const sendEmails = async (client: SES, serviceDomain: string, emailAddresses: string[]): Promise<void> => {
    ...
    const mailOptions: Options[] = emailAddresses.map((emailAddress) => ({
        ...
        to: emailAddress,
        subject: 'One or more multi-operator products have been completed',
        text: 'One or more multi-operator products have been completed',
        html: generateEmailTemplate(serviceDomain),
    }));

    const emailPromises = mailOptions.map((mailOption) => mailTransporter.sendMail(mailOption));
